        ...
        if self.training:
            ...
            if not self.local_window:
                ...
                for t_start, t_end in views:
                    ...
                    weight_tensor = weight_tensor * torch.Tensor(weight_sequence).to(hidden_states.device).unsqueeze(0).unsqueeze(-1)

                    sub_norm_hidden_states = norm_hidden_states[:, t_start:t_end]
                    sub_hidden_states = hidden_states[:, t_start:t_end]
                    sub_hidden_states = self.attn_temp(sub_norm_hidden_states) + sub_hidden_states
